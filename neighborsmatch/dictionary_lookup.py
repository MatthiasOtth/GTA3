...
Link: https://github.com/tech-srl/bottleneck/blob/main/tasks/dictionary_lookup.py (19.12.2023)

Slightly adapted to fit our application.
...
class DictionaryLookupDataset(TreeDataset):
    def __init__(self, depth, seed=None):
        super(DictionaryLookupDataset, self).__init__(depth, seed)
    ...
    def get_combinations(self):
        ...
        if self.depth > 3:
            ...
            permutations = [rng.permutation(range(1, num_leaves + 1)) for _ in
                            range(per_depth_num_permutations)]
        else:
            permutations = rng.choice(list(itertools.permutations(range(1, num_leaves + 1))),
