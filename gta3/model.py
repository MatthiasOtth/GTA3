...
class AdjacencyAwareMultiHeadAttentionBatched(nn.Module):
    ...
    def forward(self, h, A, lengths, alpha):
        ...
        # Q_h[:,:,lengths:,:] = float('-inf')
        # K_h[:,:,lengths:,:] = float('-inf')
        # V_h[:,:,lengths:,:] = float('-inf')
        # compute the dot products
        attention = torch.matmul(Q_h, K_h.transpose(-1,-2)).transpose(-1,-2)
        #Optimize
        for i in range(lengths.shape[0]):
            attention[i,:,lengths[i]:] = float('-inf')
        # apply scaling and softmax to attention
        attention = self.softmax(attention / self.sqrt_out_dim)
        for i in range(lengths.shape[0]):
            attention[i,:,:,lengths[i]:] = 0
...
class GTA3Layer(nn.Module):
    ...
    def forward(self, h, A, alpha):
        ...
        print("Embeddings after attention in original model")
        print(f"Embedding sum: {h.sum()}")
        # apply the O matrix
        h = self.O(h)
...
class GTA3LayerBatched(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, phi, num_heads=8, residual=True, batch_norm=False, layer_norm=True, attention_bias=True):
        ...
        self.O = nn.Linear(out_dim * num_heads, out_dim)
        self.aa_attention = AdjacencyAwareMultiHeadAttentionBatched(in_dim=in_dim, out_dim=out_dim, phi=self.phi, num_heads=num_heads, bias=attention_bias)
        self.FFN_layer_1 = nn.Linear(out_dim, out_dim * 2)
        self.FFN_layer_2 = nn.Linear(out_dim * 2, out_dim)
...
class GTA3BaseModelBatched(L.LightningModule):
